...
function setup() {
    const imageParam = params.get('image') || 'https://egevpare.ru/wp-content/uploads/2022/06/084mc3hmqoc.jpg'
...
class Scoreboard {
	/**
	 * @type {{name: string, time: number, cols: number, rows: number}[]}
	 */
	...
	constructor() {
		try {
			this.#scoreboard = JSON.parse(localStorage.getItem('pg-scoreboard')) || [];
		} catch (error) {
			this.#scoreboard = [];
		}
	}
	...
	save(name, time) {
		this.#scoreboard.push({ name, time, cols: this.#cols, rows: this.#rows });
		this.#scoreboard.sort((a, b) => a.time - b.time);

		localStorage.setItem('pg-scoreboard', JSON.stringify(this.#scoreboard));
	}

	get() {
		return this.#scoreboard.filter(({cols, rows}) => cols == this.#cols && rows == this.#rows);
	}
}
